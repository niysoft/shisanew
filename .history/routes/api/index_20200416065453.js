...
function fileFilter (req, file, cb, res){
    ...
    // res.status(SERVER_ERROR_RESPONSE_CODE).json(ServerErrorResponse)
     ...
     res.status(SERVER_ERROR_RESPONSE_CODE).json(ServerErrorResponse)//userController.handleErrorServer(null, res, "Error! Something went wrong. Please retry action")
    }
  }
