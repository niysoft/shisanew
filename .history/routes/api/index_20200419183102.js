    ...
    switch (filetyp) {
        case "image":
            if (typeArray[0] == "image") {//typeArray[0] == "video" ||
                cb(null, true);
                req.headers.error = "Only image files are allowed. Click other tabs for more upload file types"
            } else {
                cb(null, false);
            };
            break;
        case "video":
            if (typeArray[0] == "video") {//typeArray[0] == "video" ||
                cb(null, true);
                req.headers.error = "Only video files are allowed. Click other tabs for more upload file types"
            } else {
                cb(null, false);
            };
            break;
        case "audio":
            if (typeArray[0] == "audio") {//typeArray[0] == "video" ||
                req.headers.error = "Only audio files are allowed. Click other tabs for more upload file types"
                cb(null, true);
            } else {
                cb(null, false);
            };
