...
const storage = multer.diskStorage({
    ...
    filename: function (req, file, cb) {
        ...
        let filename = req.headers.email + "_" + req.headers.caseid + "_" + req.headers.incidentid + "_" + Date.now() + path.extname(file.originalname)//req.cookies.incidentId//JSON.parse(req.cookies.userData)
...
routes.post('/upload_files', upload.array('images', 10), function (req, res, next) {
    ...
    if (!req.body && !req.files) {
        ...
        originalFileName.forEach(element => {
            element.caseId = eq.body.caseId
            element.incidentId = eq.body.caseId
            element.userId = eq.body.userId
        });
